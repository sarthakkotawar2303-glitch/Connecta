      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 text-white placeholder-slate-500 px-4 py-2.5 rounded-xl focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none transition"
          />
        </div>
            ...
            {searchResults.map((u) => (
              ...
              >
                {(!u.pic || u.pic.includes("anonymous-avatar-icon")) ? (
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shadow-sm flex-shrink-0 ${getAvatarBgColor(u.username)}`}>
                  ...
                  <img
                    src={u.pic}
                    alt={u.username}
                    className="w-8 h-8 rounded-full object-cover border border-[#334155]/40 shadow-sm flex-shrink-0"
                  />
                )}
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {selectedUsers.map((u) => (
              <span
                ...
                className="flex items-center gap-1 bg-sky-600/20 border border-sky-500/30 text-sky-300 text-xs px-2.5 py-1 rounded-full cursor-pointer hover:bg-red-500/20 hover:text-red-300 hover:border-red-500/30 transition"
              >
                {u.username}
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-sky-600 hover:bg-sky-500 text-white py-2.5 rounded-xl font-semibold transition disabled:opacity-50 flex items-center justify-center cursor-pointer shadow-lg"
        >
          {loading
